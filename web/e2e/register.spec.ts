...
test('邀请码注册 → 登录落书架 → 改昵称 → header 显示新昵称', async ({ page }) => {
  ...
  // 应用壳左栏底部用户区应显示注册时填的昵称。
  await expect(page.getByText(initialNickname).first()).toBeVisible()
  // 经左栏「个人资料」链接进入资料页（/profile 用独立 AppHeader，非壳）。
  ...
  // 4) 断言 /profile 头部用户菜单显示新昵称（refreshUser 后 auth 状态已更新）。
